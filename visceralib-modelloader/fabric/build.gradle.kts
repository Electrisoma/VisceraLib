...
loom {
    accessWidenerPath = common.project.file("../../src/main/resources/accesswideners/${currentMod.mc}-visceralib_modelloader.accesswidener")
    ...
    mixin {
        defaultRefmapName = "visceralib_modelloader.refmap.json"
    }
}

tasks.named<ProcessResources>("processResources") {
    val awFile = project(commonProjectPath).file("src/main/resources/accesswideners/${currentMod.mc}-visceralib_modelloader.accesswidener")

    from(awFile.parentFile) {
        include(awFile.name)
        rename(awFile.name, "visceralib_modelloader.accesswidener")
        into("")
    }
}